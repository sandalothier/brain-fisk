...
import com.brain.fisk.config.KafkaProperties;
...
class FiskKafkaResourceIT {
    ...
    void setup() {
        ...
        FiskKafkaResource kafkaResource = new FiskKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/fisk-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/fisk-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
